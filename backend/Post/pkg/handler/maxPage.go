...
func MaxPage(uc usecase.UseCase, repo repository.Repository) fiber.Handler {
	return func(ctx *fiber.Ctx) error {
		category := ctx.Query("category", "all")
		filter := ctx.Query("filter", "without")
		number := ctx.QueryInt("number", 20)

		maxPage, err := uc.MaxPage(category, filter, number, repo)
		if err != nil {
			return ctx.Status(fiber.StatusInternalServerError).JSON(err.Error())
		}

		return ctx.Status(fiber.StatusOK).JSON(model.MaxPageResponse{MaxPage: maxPage})
	}
}
