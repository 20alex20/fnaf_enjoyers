...
func GetRejectedPosts(repo repository.Repository) fiber.Handler {
	return func(ctx *fiber.Ctx) error {
		...
		postReqUrl := fmt.Sprintf("http://localhost:3001/posts/rejected?id=%s", userID)
		...
		posts := make([]model.RejectedPostResponse, 0)
