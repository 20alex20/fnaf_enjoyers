...
func UnsetLike(uc usecase.UseCase, repo repository.Repository) fiber.Handler {
	return func(ctx *fiber.Ctx) error {
		...
		err = repo.UnsetLike(userID, postID)
		if err != nil {
			return ctx.Status(fiber.StatusInternalServerError).JSON(err.Error())
		}

		url := fmt.Sprintf("http://localhost:3001/post/unset-like?post_id=%s", postID)
